...
def connect(url_rhevm, username, password):
    ...
    """
    try:
      api = API(url=url_rhevm, username=username, password=password, insecure="True")
      return api
...
def create_vm_to_export(vm, id_snap, api, new_name):
    try:
        ...
        api.vms.add(params.VM(name=new_name, snapshots=snapshots, cluster=cluster, template=api.templates.get(name="Blank")))
    except Exception as e:
        print e.message
        exit(-1)
...
def import_vm(name, api, export, cluster, storage_name):
    ...
    """
    try:
       api.storagedomains.get(export).vms.get(name).import_vm(params.Action(
           storage_domain=api.storagedomains.get(storage_name), cluster=api.clusters.get(name=cluster)))
...
def print_info(api,name):
    ...
    #for nic in vm1.nics.list():
    #    count += 1
    #    print "NIC %s MAC: %s" % (count, nic.mac.address)
    #count = 0

    for hd in vm1.disks.list():
        print "DISKS: %s %s %s %s" % (count, hd.name, hd.id, hd.size)
        #print "DISKS: %s %s" % (count, hd.name)
        count += 1
